...
$imgPrimary = filter_input(INPUT_POST, 'imgPrimary', FILTER_VALIDATE_INT);
 ...
 if($imageCheck){
  $message = '<p class="display_error">An image by that name already exists.</p>';
 } elseif (empty($invId) || empty($imgName)) {
  $message = '<p class="display_error">You must select a vehicle and image file for the vehicle.</p>';
 } else {
  ...
  if ($result) {
   $message = '<p class="display_sucess">The upload succeeded.</p>';
  } else {
   $message = '<p class="display_error">Sorry, the upload failed.</p>';
  }
 }
...
if ($remove) {
 $message = "<p class='display_sucess'>$filename was successfully deleted.</p>";
} else {
 $message = "<p class='display_error'>$filename was NOT deleted.</p>";
}
...
header('Location: /phpmotors/uploads/index.php');
    ...
    $imageDisplay = '<p class="display_error">Sorry, no images could be found.</p>';
    }
